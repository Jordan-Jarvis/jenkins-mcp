...
class HTTPStreamingServer:
    ...
    def __init__(self, config: Optional[MCPConfig] = None, port: int = 8000, host: str = "0.0.0.0"):
        self.config = config
        self.port = port
        self.host = host
    ...
    async def _stream_response(self, request: dict, session_id: str):
        ...
        yield {
            "event": "message",
            "data": '{"type": "acknowledgment", "request_id": ' + str(request.get("id")) + '}'
        }
    ...
    async def _server_initiated_stream(self, session_id: str, last_event_id: Optional[str] = None):
