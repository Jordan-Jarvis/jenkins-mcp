...
class DiagnosticConfig:
    ...
    failure_patterns: FailurePatternsConfig = field(default_factory=FailurePatternsConfig)
    recommendations: RecommendationsConfig = field(default_factory=RecommendationsConfig)
    build_processing: BuildProcessingConfig = field(default_factory=BuildProcessingConfig)
...
class DiagnosticConfigLoader:
    ...
    def _load_config(self) -> None:
        ...
        try:
            ...
            logger.warning(f"Diagnostic config file not found at {config_path}, using defaults")
    ...
    def _parse_config(self, yaml_data: Dict[str, Any]) -> DiagnosticConfig:
        ...
        if 'build_processing' in yaml_data:
            ...
            config.build_processing = BuildProcessingConfig(
                parallel=bp_data.get('parallel', {}),
                chunks=bp_data.get('chunks', {})
            )
        ...
        if 'summary' in yaml_data:
            ...
            config.summary = SummaryConfig(
                ...
                failure_list_template=sum_data.get('failure_list_template', "  - {job_name} #{build_number} ({status})\n"),
                overflow_message_template=sum_data.get('overflow_message_template', "  ... and {count} more failures\n"),
        ...
        if 'context' in yaml_data:
            ...
            config.context = ContextConfig(
                ...
                high_value_chunk_threshold=ctx_data.get('high_value_chunk_threshold', 0.7),
