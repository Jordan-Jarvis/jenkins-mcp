...
class TestSimpleValidation:
    ...
    async def test_jenkins_test_double_basic_functionality(self, simple_jenkins):
        ...
        response = requests.get(f"http://localhost:{simple_jenkins.port}/job/sample-job/api/json")
        assert response.status_code == 200
            ...
            try:
                ...
                if process.poll() is None:
                    ...
                    if "vector" in stderr.lower() or "pinecone" in stderr.lower():
                        pytest.skip("Server failed due to vector database issues - expected until Task 27")
                    else:
                        raise AssertionError(f"Server failed for unexpected reason: {stderr}")
    ...
    async def test_task_25_completion_validation(self):
        ...
        runner_script = Path(__file__).parent.parent.parent / "scripts/run_integration_tests.py"
