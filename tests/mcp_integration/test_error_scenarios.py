...
class TestErrorScenarios:
    ...
    async def test_jenkins_connection_failure(self):
        ...
        async with MCPTestClient("mcp_server/server.py", config) as client:
            ...
            assert any(word in error_msg for word in ["connection", "timeout", "refused", "unreachable"])
    ...
    async def test_invalid_parameter_types(self, test_environment):
        ...
        async with MCPTestClient("mcp_server/server.py", config) as client:
            ...
            assert "error" in result
            error_msg = result["error"]["message"].lower()
            assert "parameter" in error_msg or "type" in error_msg or "invalid" in error_msg
    ...
    async def test_concurrent_tool_calls_error_isolation(self, test_environment):
        ...
        async with MCPTestClient("mcp_server/server.py", config) as client:
            ...
            tasks = [
                client.call_tool("trigger_build_async", {"job_name": "sample-job"}),  # Good
                client.call_tool("trigger_build_async", {"job_name": "non-existent"}),  # Bad
                client.call_tool("get_job_parameters", {"job_name": "sample-job"}),  # Good
                client.call_tool("get_log_context", {"job_name": "sample-job", "build_number": 99999})  # Bad
            ]
    ...
    async def test_vector_database_connection_failure(self, test_environment):
        ...
        async with MCPTestClient("mcp_server/server.py", config) as client:
            ...
            assert "error" in result
            error_msg = result["error"]["message"].lower()
            assert any(word in error_msg for word in ["connection", "vector", "database", "qdrant"])
    ...
    async def test_cache_directory_permissions(self):
        ...
        async with MCPTestClient("mcp_server/server.py", config) as client:
            ...
            if "error" in result:
                error_msg = result["error"]["message"].lower()
                assert any(word in error_msg for word in ["permission", "cache", "directory", "write"])
    ...
    async def test_unknown_tool_call(self, test_environment):
        ...
        async with MCPTestClient("mcp_server/server.py", config) as client:
            ...
            assert "error" in result
            error_msg = result["error"]["message"].lower()
            assert "unknown" in error_msg or "not found" in error_msg or "invalid" in error_msg
    ...
    async def test_server_initialization_failure(self):
        ...
        try:
            ...
            assert "initialization" in str(e).lower() or "connection" in str(e).lower() or "timeout" in str(e).lower()
