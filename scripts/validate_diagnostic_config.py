...
try:
    from mcp_server.diagnostic_config import get_diagnostic_config, reload_diagnostic_config
except ImportError:
    print("❌ Cannot import diagnostic config module. Make sure you're running from the project root.")
    sys.exit(1)
...
def validate_performance_settings(config: Dict[str, Any]) -> bool:
    ...
    if truncation >= max_tokens:
        issues.append(f"truncation_threshold {truncation} >= max_tokens_total {max_tokens}")
...
def test_config_loading() -> bool:
    ...
    try:
        ...
        print(f"✅ Configuration loading: {len(queries)} search queries, "
...
def find_config_files() -> List[Path]:
    ...
    # Default bundled
    bundled_config = project_root / "mcp_server" / "diagnostic_config" / "diagnostic-parameters.yml"
...
def main():
    ...
    if all_valid:
        ...
        # Provide next steps
        print("\n💡 Next Steps:")
        print("1. Review the quick reference: config/diagnostic-parameters-quick-reference.md")
