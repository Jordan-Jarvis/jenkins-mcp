...
class MCPStdioProxy:
    ...
    async def send_request(self, method: str, params: Dict[str, Any] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            await self.start_mcp_server()
            raise HTTPException(status_code=500, detail=f"MCP server communication error: {e}")
...
mcp_command = ["docker", "exec", "-i", mcp_server_host, "python3", "-m", "mcp_server.server"]
...
async def initialize():
    ...
    try:
        response = await proxy.send_request("initialize", {
...
async def call_tool(request: Request):
    ...
    try:
        ...
        response = await proxy.send_request("tools/call", {
            "name": tool_name,
            "arguments": arguments
        })
        return response
...
async def read_resource(request: Request):
    ...
    try:
        ...
        response = await proxy.send_request("resources/read", {
            "uri": uri
        })
        return response
    except Exception as e:
